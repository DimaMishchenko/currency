...
struct WidgetTests {
  ...
  @Test func listOwnershipAndLocalPositionAreIndependentOfResolution() {
    ...
    let success = ResolvedCurrencySelection(codes: custom, location: location, status: .available)
    #expect(success.canonical == custom)
    #expect(success.codes == ["JPY", "@local:CZK", "CZK"])
    #expect(success.localCode == "CZK")
    let failed = ResolvedCurrencySelection(codes: custom, location: location, status: .failed)
    ...
    for status in [WidgetLocationStatus.notDetermined, .denied, .restricted, .removed] {
      let unresolved = ResolvedCurrencySelection(codes: custom, location: location, status: status)
      #expect(unresolved.codes == custom)
      #expect(unresolved.localCode == nil)
    }
    #expect(
      ResolvedCurrencySelection(codes: custom, location: nil, status: .failed).codes == custom)
