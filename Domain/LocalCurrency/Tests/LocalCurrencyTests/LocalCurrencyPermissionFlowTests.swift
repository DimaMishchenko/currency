  ...
  struct WidgetOnboardingTests {
    ...
    private func withStore(_ body: (LocalCurrencyStore) async throws -> Void) async throws {
      let directory = FileManager.default.temporaryDirectory.appendingPathComponent(
        UUID().uuidString)
      defer { try? FileManager.default.removeItem(at: directory) }
      try await body(LocalCurrencyStore(directory: directory))
    }
    ...
    private func settle(_ controller: LocalCurrencyController) async throws {
    ...
    @Test func authorizationCallbackCannotBypassPendingServicesCheck() async throws {
      try await withStore { store in
        ...
        let controller = LocalCurrencyController(manager: manager, store: store, reloadWidgets: {})
        controller.update()
    ...
    @Test func failedCacheDoesNotPresentReadyState() async throws {
      try await withStore { store in
        ...
        let cached = LocalCurrencyController(
          manager: manager, store: store, servicesEnabled: { true }, reloadWidgets: {})
    ...
    @Test func geocoderFailureKeepsRecoveryAvailable() async throws {
      try await withStore { store in
        ...
        let controller = LocalCurrencyController(
          ...
          countryLookup: { _ in throw URLError(.notConnectedToInternet) }, reloadWidgets: {})
        controller.update()
        controller.locationManager(manager, didUpdateLocations: [coarseLocation])
    ...
    @Test func enablingServicesInSettingsOffersExplicitRetry() async throws {
      try await withStore { store in
        ...
        manager.permission = .authorizedWhenInUse
        let controller = LocalCurrencyController(
          manager: manager, store: store, servicesEnabled: { enabled }, reloadWidgets: {})
        controller.update()
