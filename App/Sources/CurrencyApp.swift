...
final class ConverterModel {
  ...
  func refresh(force: Bool = false) async {
    guard !refreshing, force || Date().timeIntervalSince(book.checkedAt ?? .distantPast) >= 1800
    else { return }
    refreshing = true
    defer { refreshing = false }
    let result = await service.refresh(previous: book, force: force)
    book = result.book; warning = result.warning
    do { try SharedStore.rates.save(book); WidgetCenter.shared.reloadAllTimelines() } catch {
      warning = "Rates updated, but couldn’t be saved for offline use."
    }
  }
}
...
struct ConverterView: View {
  ...
  private var amountLabel: String {
    model.input.amount.replacingOccurrences(of: ".", with: Locale.current.decimalSeparator ?? ".")
  }
  var body: some View {
    NavigationStack {
      ScrollView {
        VStack(spacing: 0) {
          ...
          HStack {
            ...
            Text(String(format: "%02d", model.input.destinations.count))
          ...
          Button {
            ...
            HStack(spacing: 6) {
              ...
              Text(
                model.book.quotes.isEmpty
                  ? "Connect to download rates" : "Saved rates · available offline")
      ...
      .sheet(item: $picker) { purpose in
        CurrencyChooser(
          ...
          selected: purpose == .source
            ? [model.input.from] : model.input.destinations + [model.input.from],
          available: Set(model.book.quotes.keys)
        ) { code in
          withAnimation(motion) {
            if purpose == .source {
              model.input.changeSource(code)
            } else {
              model.input.setDestinations(model.input.destinations + [code])
            }
          }
        }
      }
      ...
      .sheet(item: $detail) { selection in
        CurrencyDetailView(code: selection.id, reference: model.input.from, book: model.book)
      }
      .task {
  ...
  private var header: some View {
    HStack {
      ...
      Menu {
        Button("Refresh rates", systemImage: "arrow.clockwise") {
    ...
    VStack(alignment: .leading, spacing: 15) {
      HStack {
        Button {
          ...
          HStack(spacing: 9) {
            Text(Currency.flag(model.input.from)).font(.system(size: 23))
              .matchedGeometryEffect(id: "flag-" + model.input.from, in: currencyMotion)
              .accessibilityHidden(true)
            Text(model.input.from).font(.system(.subheadline, weight: .semibold))
              .matchedGeometryEffect(id: model.input.from, in: currencyMotion)
        ...
        Spacer()
        Text("BASE AMOUNT").font(.system(size: 9, weight: .medium)).tracking(1.5)
          .foregroundStyle(.secondary)
      }
      Button {
      ...
      .buttonStyle(.plain).accessibilityLabel("Edit amount in \(model.input.from)")
      .accessibilityValue(amountLabel)
      HStack {
        Text(Currency.name(model.input.from)).font(.subheadline).foregroundStyle(.secondary)
        Spacer()
        if editingAmount {
          Text("Editing").font(.caption2).foregroundStyle(accent).transition(.opacity)
        }
      }
    }
  }
  private func destinationRow(_ code: String) -> some View {
    ...
    return HStack(spacing: 0) {
      Button {
        ...
        HStack(spacing: 13) {
          Text(Currency.flag(code)).font(.system(size: 28)).frame(width: 38)
          ...
          Spacer(minLength: 12)
          Text(Currency.format(value, code: code))
      ...
      .contextMenu {
        Button("Details & history", systemImage: "chart.xyaxis.line") {
          detail = CurrencyDetailSelection(id: code)
        }
        Button("Use as base", systemImage: "arrow.up") {
          withAnimation(motion) { model.input.useAsBase(code, book: model.book) }
        }
        .disabled(value == nil)
        Button("Copy amount", systemImage: "doc.on.doc") {
          UIPasteboard.general.string = Currency.format(value, code: code)
        }
        .disabled(value == nil)
        Text(model.book.details(from: model.input.from, to: code))
        Button("Remove", systemImage: "minus.circle", role: .destructive) {
          withAnimation(motion) {
            model.input.setDestinations(model.input.destinations.filter { $0 != code })
          }
        }
      }
      Button {
  ...
  private var inputDock: some View {
    GlassEffectContainer(spacing: 30) {
      if editingAmount {
        VStack(spacing: 4) {
          HStack {
            Text("\(Currency.flag(model.input.from))  \(model.input.from)")
            ...
            ForEach(
              ...
              GridRow {
                ForEach(row, id: \.self) { item in
                  Button {
                    ...
                    Group {
                      if item == "⌫" {
                        ...
                        Text(item == "." ? Locale.current.decimalSeparator ?? "." : item)
                  ...
                  .accessibilityLabel(
                    item == "⌫" ? "Delete digit" : item == "." ? "Decimal separator" : item)
                }
              }
            }
          }
        ...
        HStack(spacing: 0) {
          Button {
            ...
            Label("Enter amount", systemImage: "keyboard").font(.subheadline.weight(.medium))
          ...
          .buttonStyle(.plain).accessibilityLabel("Add currency")
        }
  ...
  private var rateInformation: some View {
    NavigationStack {
      List {
        Section {
          Text(
            "Daily reference rates, saved on your device. Every conversion uses the latest downloaded rates; bank fees aren’t included."
          )
          if model.book.fetchedAt != .distantPast {
            LabeledContent(
              "Last checked",
              value: model.book.fetchedAt.formatted(date: .abbreviated, time: .shortened))
          }
          ...
          ForEach([model.input.from] + model.input.destinations, id: \.self) { code in
            LabeledContent("\(Currency.flag(code)) \(code)") {
              VStack(alignment: .trailing, spacing: 3) {
                ...
                Text(model.book.quotes[code]?.source ?? "Unavailable").font(.caption)
                  .foregroundStyle(.secondary)
              }
            }
          }
        ...
        Section("Home screen") {
          Text(
            "Widgets share this amount and currency list. The large widget includes a keypad. iOS controls widget input processing and refresh timing."
          )
        }
        Section("Sources") {
          Text(
            "Frankfurter v2 for daily fiat rates, with direct ECB fallback. Fawaz supplies daily crypto and backup rates. Coinbase public market data is checked every 30 minutes while active, falling back to daily crypto rates if unavailable. iOS schedules widget refreshes. No accounts or API keys."
          )
        }
      }
      .navigationTitle("About these rates").navigationBarTitleDisplayMode(.inline)
      .toolbar {
        ToolbarItem(placement: .confirmationAction) { Button("Done") { showInfo = false } }
      }
    }
  }
}
...
struct CurrencyChooser: View {
  ...
  var body: some View {
    NavigationStack {
      List {
        ForEach([false, true], id: \.self) { crypto in
          Section(crypto ? "Crypto" : "Currencies") {
            ForEach(
              Currency.codes.filter {
                Currency.crypto.contains($0) == crypto
                  && (search.isEmpty
                    || "\($0) \(Currency.name($0))".localizedCaseInsensitiveContains(search))
              }, id: \.self
            ) { code in
              Button {
                choose(code); dismiss()
      ...
      .navigationTitle(purpose == .source ? "Base currency" : "Add currency")
    ...
    NavigationStack {
      List {
        Section {
          ...
          .onDelete { offsets in
            var list = input.destinations; list.remove(atOffsets: offsets);
            input.setDestinations(list)
          }
          .onMove { source, destination in
            var list = input.destinations; list.move(fromOffsets: source, toOffset: destination);
            input.setDestinations(list)
          }
        } footer: {
          Text("Drag to reorder. Your widgets follow the same order.")
        }
      }
      .environment(\.editMode, .constant(.active)).navigationTitle("Your currencies")
      .navigationBarTitleDisplayMode(.inline)
