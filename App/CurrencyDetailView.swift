...
struct CurrencyDetailView: View {
    ...
    private var quote: String { Currency.crypto.contains(code) ? "USD" : reference == code || Currency.crypto.contains(reference) ? (code == "EUR" ? "USD" : "EUR") : reference }
    ...
    var body: some View {
        NavigationStack {
                ...
                VStack(alignment: .leading, spacing: 24) {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        Text(rateLabel(book.convert(1, from: code, to: quote)))
                            .font(.system(size: 46, weight: .light, design: .rounded)).monospacedDigit().lineLimit(1).minimumScaleFactor(0.4)
                        Text(book.details(from: code, to: quote)).font(.caption).foregroundStyle(.secondary)
                        if let observed = book.quotes[code]?.observedAt {
                            Text("Last trade \(observed.formatted(date: .abbreviated, time: .shortened))").font(.caption).foregroundStyle(.secondary)
                        }
                    }
                    Divider()
                    HStack {
                    ...
                    if let series, !series.points.isEmpty {
                        VStack(alignment: .leading, spacing: 4) {
                            if let selected {
                                Text("\(rateLabel(Decimal(selected.value))) \(quote)").font(.title3.monospacedDigit())
                        ...
                        Chart(series.points) { point in
                            ...
                            if selectedDate != nil, selected?.id == point.id {
                                ...
                                PointMark(x: .value("Date", point.date), y: .value(quote, point.value)).foregroundStyle(Color.accentColor)
                            }
                        }
                        ...
                        .frame(height: 220)
                        .accessibilityLabel("\(days == 0 ? "All available" : days == 365 ? "One year" : "\(days) day") \(code) to \(quote) history")
                        Text(series.source).font(.caption).foregroundStyle(.secondary)
                        if let first = series.points.first, let last = series.points.last {
                            Text("\(dayLabel(first.date)) – \(dayLabel(last.date))").font(.caption2).foregroundStyle(.secondary)
                        }
                        Text("Saved \(series.fetchedAt.formatted(date: .abbreviated, time: .shortened))").font(.caption2).foregroundStyle(.secondary)
                    } else if !loading {
                        ContentUnavailableView("No history available", systemImage: "chart.xyaxis.line", description: Text("Try another range or return when connected."))
                    }
                    if let message { Text(message).font(.caption).foregroundStyle(.secondary) }
                    if days == 0 { Text("Max shows all history available from this provider for this pair, grouped monthly. It may begin after the currency launched.").font(.caption).foregroundStyle(.secondary) }
                    Text(Currency.crypto.contains(code) ? "Chart shows completed daily Coinbase trades in USD. Missing trading days have no observations. Current conversion rates may use Fawaz daily fallback." : "Daily reference rates. Weekends and publication gaps may have no observations. History uses Frankfurter; the converter keeps its ECB fallback.")
                        .font(.caption).foregroundStyle(.secondary)
                }.padding(26)
            }
            ...
            .task(id: days) {
                ...
                let result = await HistoryService(directory: SharedStore.directory).load(base: code, quote: quote, days: days)
