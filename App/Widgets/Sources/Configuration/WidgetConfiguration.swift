...
struct CurrencyQuery: EntityStringQuery {
  ...
  func suggestedEntities() async throws -> IntentItemCollection<WidgetCurrency> {
    currencySections(input: readInput(), make: WidgetCurrency.init)
  }
...
struct CashQuery: EntityStringQuery {
  ...
  func suggestedEntities() async throws -> IntentItemCollection<CashCurrency> {
    currencySections(input: readInput(), cashOnly: true, make: CashCurrency.init)
  }
...
protocol SuiteConfiguration: WidgetConfigurationIntent {
  func specification(
    kind: String, input: ConverterState, location: WidgetLocation?, status: WidgetLocationStatus
  ) -> WidgetSpec
}
  ...
  func specification(
    ...
    var spec = WidgetSpec(
      ...
      codes: WidgetSelection.board(
        base: list == .synchronized ? input.source : base.id,
        targets: (list == .synchronized
          ? WidgetSelection.appConfiguration(input)
          : (currencies?.map(\.id) ?? []))
      ),
      amount: list == .synchronized ? input.amount : amount,
      location: location, status: status)
