...
/// Interactive banknote and metal-weight reference widget.
public struct CashView: View {
  @Environment(\.locale) private var locale
  let entry: SuiteEntry
  /// Creates a reference-value widget with the entry’s preset currencies.
  public init(entry: SuiteEntry) { self.entry = entry }

  private var base: String { entry.spec.codes.first ?? "EUR" }

  private var target: String { WidgetSelection.currency(entry.spec.codes.last ?? "USD") }
  ...
  /// The shared reference widget content.
    ...
    VStack(alignment: .leading, spacing: AppStyle.Space.small) {
      ...
      Text(
        WidgetPresets.metals.contains(base)
          ? .WidgetPresentation.metalEstimate
          : WidgetPresets.isBanknote(base)
            ? .WidgetPresentation.banknoteEstimate : .WidgetPresentation.genericEstimate
      )
...
/// Pocket-rate and mental-math layouts built from one currency pair.
public struct AnchorView: View {
  @Environment(\.locale) private var locale
  let entry: SuiteEntry
  var mental = false
  /// Chooses the pocket conversion or mental-math presentation.
  public init(entry: SuiteEntry, mental: Bool = false) { self.entry = entry; self.mental = mental }
      ...
      if target == WidgetSelection.localID {
        ...
        if mental, let rule = WidgetMath.rule(rate: rate) {
          ...
          VStack(spacing: AppStyle.Space.xs) {
            ...
            Divider()
            pocketRow(
              code: target, isLocal: entry.spec.usesLocation,
              amount: "≈ " + CurrencyDisplay.format(anchor * rate, code: target, locale: locale))
          }
        }
      } else {
        Text(.WidgetPresentation.openToLoad).font(AppStyle.font(.headline))
      }
      if mental { Spacer(minLength: 0) }
      WidgetFooter(entry: entry)
    }
