...
@Test func longHistoryWindowsAndMonthlyCloses() {
    ...
    let points = [HistoryPoint(date: start, value: 1), HistoryPoint(date: start.addingTimeInterval(86400), value: 2), HistoryPoint(date: start.addingTimeInterval(40 * 86400), value: 3)]
...
private actor LongHistoryHTTP: HTTPClient {
    ...
    func get(_ url: URL) async throws -> Data {
        ...
        let raw = query.first { $0.name == "start" }!.value!
        let start = ISO8601DateFormatter().date(from: raw)!.timeIntervalSince1970
        return try JSONEncoder().encode([[start + 86400, 1, 5, 2, 3, 1], [start + 172800, 1, 5, 2, 4, 1]])
    }
}
@Test func yearlyCryptoFetchesEveryPageAndCaches() async {
    let directory = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
    defer { try? FileManager.default.removeItem(at: directory) }
    let client = LongHistoryHTTP()
    let service = HistoryService(directory: directory, client: client)
    let now = ISO8601DateFormatter().date(from: "2026-01-02T00:00:00Z")!
...
private actor MaxFiatHTTP: HTTPClient {
    ...
    func get(_ url: URL) async throws -> Data {
        urls.append(url)
        return Data(#"[{"date":"1999-01-01","base":"EUR","quote":"USD","rate":1.1},{"date":"2026-01-01","base":"EUR","quote":"USD","rate":1.2}]"#.utf8)
    }
}
@Test func maxFiatRequestsMonthlyHistoryAndCachesForOneDay() async {
    let directory = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
    defer { try? FileManager.default.removeItem(at: directory) }
    ...
    let url = await client.urls.first!
    let query = URLComponents(url: url, resolvingAgainstBaseURL: false)!.queryItems!
...
private struct Stub: RateProvider {
    ...
    func fetch() async throws -> [String: Quote] { guard let quotes else { throw RateError.unavailable }; return quotes }
}

@Test func frankfurterValidationAndExpandedCatalog() throws {
    let data = Data(#"[{"date":"2026-01-02","base":"EUR","quote":"USD","rate":1.2},{"date":"2026-01-02","base":"EUR","quote":"UAH","rate":50}]"#.utf8)
    ...
    #expect(throws: (any Error).self) { try FrankfurterProvider.decode(Data(#"[{"date":"2026-01-02","base":"USD","quote":"EUR","rate":1}]"#.utf8)) }
}
...
@Test func coinbaseInversionAndStaleness() throws {
    ...
    #expect(throws: (any Error).self) { try CoinbaseProvider.decode(data, now: now.addingTimeInterval(7200)) }
    #expect(throws: (any Error).self) { try CoinbaseProvider.decode(Data(#"{"price":"0","time":"2026-01-02T12:00:00Z"}"#.utf8), now: now) }
}
@Test func coinbaseFailureRestoresDailyAndDailyCacheIsRetained() async {
    let daily = ["EUR": Quote(1, published: day, source: "daily"), "BTC": Quote(2, published: day, source: "Fawaz · daily")]
    let live = ["BTC": Quote(3, published: day, source: "Coinbase", observedAt: .now)]
    let fresh = await RateService(fiat: Stub(quotes: daily), daily: Stub(quotes: daily), crypto: Stub(quotes: live)).refresh(previous: RateBook())
    #expect(fresh.book.quotes["BTC"]?.value == 3)
    #expect(fresh.book.dailyQuotes?["BTC"]?.value == 2)
    let failed = await RateService(fiat: Stub(quotes: nil), daily: Stub(quotes: nil), crypto: Stub(quotes: nil)).refresh(previous: fresh.book)
...
@Test func historySortsAndUsesCompletedClose() throws {
    ...
    let points = try HistoryService.decodeCandles(rows, start: Date(timeIntervalSince1970: 86400), end: Date(timeIntervalSince1970: 300000))
    #expect(points.map(\.value) == [4, 5])
    #expect(throws: (any Error).self) { try HistoryService.decodeCandles(Data("[[1,2]]".utf8), start: .distantPast, end: .now) }
    let fiat = Data(#"[{"date":"2026-01-03","base":"USD","quote":"EUR","rate":0.9},{"date":"2026-01-02","base":"USD","quote":"EUR","rate":0.8}]"#.utf8)
...
private actor HistoryHTTP: HTTPClient {
    ...
    func get(_ url: URL) async throws -> Data {
        ...
        return Data(#"[{"date":"2026-01-02","base":"USD","quote":"EUR","rate":0.8},{"date":"2026-01-03","base":"USD","quote":"EUR","rate":0.9}]"#.utf8)
    }
}
@Test func historyCachesAndFallsBackOffline() async throws {
    ...
    let offline = await service.load(base: "USD", quote: "EUR", days: 30, now: Date().addingTimeInterval(86400))
    #expect(offline.series?.points == first.series?.points)
    #expect(offline.message != nil)
}
@Test func crossRatesAndMissingCurrency() {
    let book = RateBook(quotes: ["EUR": Quote(1, published: day, source: "test"), "USD": Quote(2, published: day, source: "test"), "BTC": Quote(Decimal(string: "0.00002")!, published: day, source: "test")])
...
@Test func xmlParsing() throws {
    let xml = Data("<gesmes:Envelope xmlns:gesmes='urn:test'><Cube><Cube time='2026-01-02'><Cube currency='USD' rate='1.25'/></Cube></Cube></gesmes:Envelope>".utf8)
    ...
    #expect(throws: (any Error).self) { try ECBProvider.decode(Data("<Cube time='2026-01-02'><Cube currency='USD' rate='0'/></Cube>".utf8)) }
}
@Test func dailyParsing() throws {
    ...
    #expect(throws: (any Error).self) { try FawazProvider.decode(Data(#"{"date":"bad","eur":{"usd":1,"btc":1}}"#.utf8)) }
}
@Test func fallbackAndOffline() async {
    let old = RateBook(quotes: ["USD": Quote(2, published: day, source: "saved")], fetchedAt: Date(timeIntervalSince1970: 10))
    let offline = await RateService(fiat: Stub(quotes: nil), daily: Stub(quotes: nil), crypto: nil).refresh(previous: old)
    #expect(offline.book.quotes == old.quotes)
    #expect(offline.book.fetchedAt == old.fetchedAt)
    #expect(offline.warning != nil)
    let result = await RateService(fiat: Stub(quotes: nil), daily: Stub(quotes: ["USD": Quote(3, published: "2026-01-03", source: "fallback")]), crypto: nil).refresh(previous: old)
...
@Test func keypad() {
    var input = InputState(); input.press("AC"); input.press("0"); input.press(","); input.press("."); input.press("5")
...
@Test func cacheRoundTripAndCorruption() throws {
    ...
    let book = RateBook(quotes: ["BTC": Quote(Decimal(string: "0.000012345678")!, published: day, source: "daily")])
    try store.save(book)
...
@Test func promoteCurrencyPreservesValueAndOrder() {
    var input = InputState(); input.amount = "100"
    input.setDestinations(["USD", "GBP", "JPY"])
    let book = RateBook(quotes: ["EUR": Quote(1, published: day, source: "test"), "USD": Quote(Decimal(string: "1.25")!, published: day, source: "test")])
