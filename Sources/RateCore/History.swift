...
public actor HistoryService {
    ...
    public init(directory: URL, client: any HTTPClient = NetworkClient(timeout: 30)) { self.directory = directory; self.client = client }
    public func load(base: String, quote: String, days: Int, now: Date = .now) async -> HistoryResult {
        ...
        guard Currency.codes.contains(base), Currency.codes.contains(quote), [7, 30, 90, 365, 0].contains(days), base != quote else { return HistoryResult(series: nil, message: "History is unavailable for this pair.") }
        let file = directory.appendingPathComponent("history-\(base)-\(quote)-\(days).json")
        let cached = (try? Data(contentsOf: file)).flatMap { try? JSONDecoder().decode(HistorySeries.self, from: $0) }
        if let cached, now.timeIntervalSince(cached.fetchedAt) < (days == 0 ? 86400 : 21600) { return HistoryResult(series: cached, message: nil) }
        do {
            let isCrypto = Currency.crypto.contains(base)
            var calendar = Calendar(identifier: .gregorian)
            calendar.timeZone = TimeZone(secondsFromGMT: 0)!
            let start = days == 0
                ? calendar.date(from: DateComponents(year: isCrypto ? 2009 : 1948, month: 1, day: 1))!
                : days == 365 ? calendar.date(byAdding: .year, value: -1, to: now)!
                : now.addingTimeInterval(-Double(days) * 86400)
            var components = URLComponents(string: isCrypto ? "https://api.exchange.coinbase.com/products/\(base)-\(quote)/candles" : "https://api.frankfurter.dev/v2/rates")!
            ...
            if isCrypto {
                ...
                for window in Self.candleWindows(start: start, end: now) {
                    ...
                    components.queryItems = [URLQueryItem(name: "granularity", value: "86400"), URLQueryItem(name: "start", value: window.start.ISO8601Format()), URLQueryItem(name: "end", value: window.end.ISO8601Format())]
                    let data = try await client.get(components.url!)
                    for point in try Self.decodeCandles(data, start: window.start, end: now) where point.date <= window.end {
                        combined[point.date] = point.value
                    }
                    if window.end < now { try await Task.sleep(for: .milliseconds(150)) }
                }
                let daily = combined.map { HistoryPoint(date: $0.key, value: $0.value) }.sorted { $0.date < $1.date }
                points = days == 0 ? Self.monthlyCloses(daily) : daily
            } else {
                components.queryItems = [URLQueryItem(name: "base", value: base), URLQueryItem(name: "quotes", value: quote), URLQueryItem(name: "from", value: String(start.ISO8601Format().prefix(10))), URLQueryItem(name: "to", value: String(now.ISO8601Format().prefix(10)))]
            ...
            let source = isCrypto ? (days == 0 ? "Coinbase · monthly last available close · UTC" : "Coinbase · daily closes · UTC") : (days == 0 ? "Frankfurter · monthly reference rates" : "Frankfurter · daily reference rates")
            ...
            } catch { return HistoryResult(series: series, message: "History loaded, but couldn’t be saved offline.") }
            return HistoryResult(series: series, message: nil)
        } catch {
            return HistoryResult(series: cached, message: cached == nil ? "History is unavailable. Your converter still works with saved rates." : "Can’t update history. Showing the saved chart.")
        }
    }
    ...
    static func monthlyCloses(_ points: [HistoryPoint]) -> [HistoryPoint] {
        ...
        for point in points.sorted(by: { $0.date < $1.date }) {
            if let month = calendar.dateInterval(of: .month, for: point.date)?.start { months[month] = point }
        }
    ...
    public static func decodeFiat(_ data: Data, base: String, quote: String) throws -> [HistoryPoint] {
        ...
        for row in rows {
            guard row.base == base, row.quote == quote, validDate(row.date), row.rate > 0, !row.rate.isNaN,
                  let date = ISO8601DateFormatter().date(from: row.date + "T00:00:00Z") else { throw RateError.invalidData }
    ...
    public static func decodeCandles(_ data: Data, start: Date, end: Date) throws -> [HistoryPoint] {
        ...
        var values: [Date: Double] = [:]
        for row in rows {
            guard row.count >= 5, row.allSatisfy(\.isFinite), row[4] > 0 else { throw RateError.invalidData }
